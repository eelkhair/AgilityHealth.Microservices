...
public class MasterTagService : IMasterTagService
{
    ...
    public async Task<MasterTagWithCategoryAndParentTagResponse> UpdateMasterTag(Guid masterTagUId, MasterTagRequest tag)
    {
         var request = new HttpRequestMessage(HttpMethod.Put, $"v1/mastertags/{masterTagUId}Name={tag.Name}&ClassName={tag.ClassName}&MasterTagCategoryUId={tag.MasterTagCategoryUId}&ParentMasterTagUId={tag.ParentMasterTagUId}");
         var response = await _httpClient.SendAsync(request);
         response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<MasterTagWithCategoryAndParentTagResponse>() ?? new MasterTagWithCategoryAndParentTagResponse();
